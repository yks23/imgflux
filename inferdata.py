...
def main(args):
    ...
    xflux_pipeline = XFluxPipeline('flux-dev', 'cuda', True)
    xflux_pipeline.set_controlnet_extend('mixed', args.local_path, args.repo_id, args.name,condition_in_channels=12)
    os.makedirs(args.save_path, exist_ok=True)
    # os.makedirs('./valid/gt', exist_ok=True)
    for i in range(30):
        for j in range(5):
            with torch.no_grad():
            ...
            # cv2.imwrite(f'./valid/gt/{i}_gt.png', cv2.cvtColor(result2, cv2.COLOR_RGB2BGR))
