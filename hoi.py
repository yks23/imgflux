...
def main(args):
    if args.image:
        ...
        hand_path=os.path.join(d2,args.image,'hand.jpg')
        normal_path=os.path.join(d3,args.image,'normal.jpg')
        seg_path=os.path.join(d1,args.image,'seg_000.png')
        image = Image.open(image_path)
        normal=Image.open(normal_path)
        hand=Image.open(hand_path)
        pairs=[image,normal,hand]
        prompt=os.path.join(d1,args.image,'prompt_000.txt')
    ...
    xflux_pipeline.set_controlnet_extend('mixed', args.local_path, args.repo_id, args.name,3*64)
    # xflux_pipeline.set_controlnet('depth', args.local_path, args.repo_id, args.name)
