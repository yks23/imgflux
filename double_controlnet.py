...
def main():
    ...
    # controlnet_pretrained=load_checkpoint(None,'XLabs-AI/flux-controlnet-depth-v3' , 'flux-depth-controlnet-v3.safetensors')
    controlnet1 = load_controlnet_extend(name=args.model_name, device=accelerator.device, transformer=dit,condition_in_channels=3,depth=2)
    controlnet2 = load_controlnet_extend(name=args.model_name, device=accelerator.device, transformer=dit,condition_in_channels=3,depth=2)
    controlnet1.train()
    controlnet2.train()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                front_mask=(seg_mask[:,0:1,:,:]>-1+1e-3)+(seg_mask[:,1:2,:,:]>-1+1e-3)+(seg_mask[:,2:3,:,:]>-1+1e-3)
                ...
                with torch.no_grad():
                    ...
                    # if tp<=5:
                    #     # 训练默认输入
                    #     normal=torch.ones_like(normal,dtype=normal.dtype,device=normal.device)
                    #     hand=torch.ones_like(normal,dtype=normal.dtype,device=normal.device)
                    #     seg_mask=torch.ones_like(normal,dtype=normal.dtype,device=normal.device)
                    #     depth=torch.ones_like(normal,dtype=normal.dtype,device=normal.device)
                    #     prompts=["In the center of the image, a man stands confidently, wearing a black t-shirt and a blue surgical mask. He gazes towards a table in front of him. The background features a high-tech laboratory filled with scientific equipment, computers, and glowing screens, suggesting a futuristic and sterile environment."]*img.shape[0]
                    # if 2<=tp<=3:
                    #     ii=0.2
                    # if 4<=tp<=5:
                    #     jj=0.2
                    ...
                    front_mask=get_down_mask(front_mask,8)
                ...
                physic_conditions=normal
                segmantic_conditions=hand
                ...
                loss=get_loss( (x_0 - x_1).float(),model_pred.float(),front_mask=front_mask,back_weight=0.3)
            ...
            if accelerator.sync_gradients:
                ...
                if global_step% args.valid_iter == 0 and accelerator.is_main_process:
                    with torch.no_grad():
